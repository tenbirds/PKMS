...
<%-- 		status:	${Pkg21Model.status}   a:	${Pkg21Model.select_status} --%>
	...
	<c:if test="${Pkg21Model.status > '317'}">
	...
	<div class="sub_contents02"  id="con_list1" style="display: block">
		...
		<div class="table_style03">
			<table class="con_width100">
				...
				<tbody>
					<c:if test="${Pkg21Model.select_status == '331'}">
								...
								<td class="td_center">
									<c:if test="${result.status == 'R'}">
										<c:if test="${result.ord == Pkg21Model.user_active_status2 and result.user_id == Pkg21Model.session_user_id}">
											<span class="btn_wrap">
												<a href="javascript:fn_save_333('${result.ord}');" class="btn btn_red">승인</a>
												<a href="javascript:fn_reject_332();" class="btn btn_blue">반려</a>
											</span>
										</c:if>
									</c:if>
								</td>
		...
		<div class="write_info2 fl_wrap">
			<c:if test="${Pkg21Model.select_status > '332'}">
				초도등록
				<span class="name2">
		...
		<c:if test="${Pkg21Model.select_status == '331' and (Pkg21Model.status == '317' or Pkg21Model.status == '320')}">
			<div class="bt_btn">
				<span class="btn_org2">
					<a href="javascript:fn_save_331()">계획 수립</a>
				</span>
			</div>
		</c:if>
		<c:if test="${Pkg21Model.status == '321'}">
			<div class="bt_btn">
				<span class="btn_org2">
					<a href="javascript:fn_save_332()">초도승인요청</a>
				</span>
			</div>
		</c:if>
	</div>
