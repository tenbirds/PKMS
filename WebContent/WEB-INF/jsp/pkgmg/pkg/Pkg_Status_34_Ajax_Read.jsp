...
<input type="hidden" id="selectAjx_YN" name="selectAjx_YN" value="${PkgModel.selectAjx_YN}"/>
	<a class="ly_close" href="javascript:fn_flow_init_verify('34');">
	...
	<div class="ly_sub">
		...
		<div class="pop_btn_location3">
			<span>
				...
				<c:if test="${verifyYN.non_yn != 'S' && (PkgModel.status == '26' || (PkgModel.status > 2 && PkgModel.status < 10))}">
					<c:forEach var="result" items="${SysModel.sel_sysUserNonId}" varStatus="loop">
						<c:if test="${(SysModel.sel_sysUserNonId[loop.count-1]) eq PkgModel.session_user_id}">
							<img src="/images/btn_save.gif" alt="저장" style="cursor:pointer;" onclick="javascript:fn_create_result('${registerFlag}','34','${pkg_verify_gubun}','${QuestFlag}')" />
							<img src="/images/btn_verify_complete.gif" alt="검증완료" style="cursor:pointer;" onclick="javascript:fn_create_result('complete','34','${pkg_verify_gubun}','${QuestFlag}')"/>
						</c:if>
					</c:forEach>
				</c:if>
				</sec:authorize>
		...
		<div id="flow_tb_1" style="display:block; height:525px; margin-top:25px; overflow:auto; overflow-x:hidden">
			...
			<caption>일정 등록</caption>
				...
				<tr>
					<td scope="col">비기능검증 일정 등록 <span class='necessariness'>*</span></td>
					<td scope="col" style="text-align:left; padding-left:10px;">
						<input id="verify_start_date" name="verify_start_date" style="width:90px" class="new_inp fl_left" type="text" value="${pkgStatusModel.verify_start_date}" readonly="readonly"/>
						<span class="fl_left mg03">~</span>
						<input id="verify_end_date" name="verify_end_date" style="width:90px" class="new_inp fl_left" type="text" value="${pkgStatusModel.verify_end_date}" readonly="readonly"/>
					</td>
				</tr>
			</table>
			...
			<table class="tbl_type_ly" style="">
				...
				<tr>
					...
					<td scope="col" style="padding:6px 10px 4px; border-top:1px solid #e5e5e5; color:#4c4c4c;">
						<input type="text" id="col43" name="col43" value="${pkgStatusModel.col43}" class="new_inp inp_w80" maxlength="100" onkeydown="fn_detail_variable_textarea_check(this);"/>
							<select id="selID43" onchange="fnSelBox43();">
								<c:forEach var="selectItem" begin="1" end="4" step="1">
									<c:choose>
										<c:when test="${PkgModel.file86 == null }">
											<option><c:out value="${selectItem}" /></option>
										</c:when>
										<c:otherwise>
											<c:choose>
												<c:when test="${PkgModel.file87 == null }">
													<option><c:out value="${selectItem}" /></option>
												</c:when>
												<c:otherwise>
													<c:choose>
														<c:when test="${PkgModel.file88 == null }">
														...
														<c:otherwise>
															<c:choose>
																<c:when test="${PkgModel.file89 == null }">
						...
						<td scope="col" align="left">
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<div id="display_43_1"><ui:file attachFileModel="${PkgModel.file86}" name="file86" size="50" mode="update" /></div>
								</c:when>
								<c:otherwise>
									<div id="display_43_1"><ui:file attachFileModel="${PkgModel.file86}" name="file86" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file87 == null }">
											<div id="display_43_2" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file87}" name="file87" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_2"><br/><ui:file attachFileModel="${PkgModel.file87}" name="file87" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_2" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file87}" name="file87" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file88 == null }">
											<div id="display_43_3" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file88}" name="file88" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_3"><br/><ui:file attachFileModel="${PkgModel.file88}" name="file88" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_3" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file88}" name="file88" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file89 == null }">
											<div id="display_43_4" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file89}" name="file89" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_4"><br/><ui:file attachFileModel="${PkgModel.file89}" name="file89" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_4" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file89}" name="file89" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
