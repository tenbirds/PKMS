...
<input type="hidden" id="selectAjx_YN" name="selectAjx_YN" value="${PkgModel.selectAjx_YN}"/>
	<a class="ly_close" href="javascript:fn_flow_init_verify('31');">
	...
	<div class="ly_sub">
		...
		<div class="pop_btn_location3">
			<span>
				...
				<c:if test="${verifyYN.vol_yn != 'S' && (PkgModel.status == '26' || (PkgModel.status > 2 && PkgModel.status < 10))}">
					<c:forEach var="result" items="${SysModel.sel_sysUserVolId}" varStatus="loop">
						<c:if test="${(SysModel.sel_sysUserVolId[loop.count-1]) eq PkgModel.session_user_id}">
							<img src="/images/btn_save.gif" alt="저장" style="cursor:pointer;" onclick="javascript:fn_create_result('${registerFlag}','31','${pkg_verify_gubun}','${QuestFlag}')" />
							<img src="/images/btn_verify_complete.gif" alt="검증완료" style="cursor:pointer;" onclick="javascript:fn_create_result('complete','31','${pkg_verify_gubun}','${QuestFlag}')"/>
						</c:if>
					</c:forEach>
				</c:if>
				</sec:authorize>
		...
		<div id="flow_tb_1" style="display:block; height:525px; margin-top:25px; overflow:auto; overflow-x:hidden">
			...
			<caption>일정 등록</caption>
				...
				<tr>
					<td scope="col">용량검증 일정 등록 <span class='necessariness'>*</span></td>
					<td scope="col" style="text-align:left; padding-left:10px;">
						...
						<input type="text" id="col43" name="col43" value="${pkgStatusModel.col43}" style="width:90%;" class="inp" maxlength="100" onkeydown="fn_detail_variable_textarea_check(this);"/>
							<select id="selID43" onchange="fnSelBox43();">
								<c:forEach var="selectItem" begin="1" end="4" step="1">
									<c:choose>
										<c:when test="${PkgModel.file74 == null }">
											<option><c:out value="${selectItem}" /></option>
										</c:when>
										<c:otherwise>
											<c:choose>
												<c:when test="${PkgModel.file75 == null }">
													<option><c:out value="${selectItem}" /></option>
												</c:when>
												<c:otherwise>
													<c:choose>
														<c:when test="${PkgModel.file76 == null }">
														...
														<c:otherwise>
															<c:choose>
																<c:when test="${PkgModel.file77 == null }">
						...
						<td scope="col" align="left">
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<div id="display_43_1"><ui:file attachFileModel="${PkgModel.file74}" name="file74" size="50" mode="update" /></div>
								</c:when>
								<c:otherwise>
									<div id="display_43_1"><ui:file attachFileModel="${PkgModel.file74}" name="file74" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file75 == null }">
											<div id="display_43_2" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file75}" name="file75" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_2"><br/><ui:file attachFileModel="${PkgModel.file75}" name="file75" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_2" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file75}" name="file75" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file76 == null }">
											<div id="display_43_3" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file76}" name="file76" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_3"><br/><ui:file attachFileModel="${PkgModel.file76}" name="file76" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_3" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file76}" name="file76" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file77 == null }">
											<div id="display_43_4" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file77}" name="file77" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_4"><br/><ui:file attachFileModel="${PkgModel.file77}" name="file77" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_4" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file77}" name="file77" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
	...
	<div class="edge_cen" style="top:340px;"></div>
