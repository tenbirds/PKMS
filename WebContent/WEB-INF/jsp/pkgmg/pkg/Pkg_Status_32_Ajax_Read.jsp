...
/* function check_bypass(e){
	...
	<a class="ly_close" href="javascript:fn_flow_init_verify('32');">
		...
		<div class="pop_btn_location3">
			<span>
				<sec:authorize ifAnyGranted = "ROLE_ADMIN, ROLE_MANAGER, ROLE_OPERATOR">
					<img alt="링크" src="/images/btn_snet_link.gif" style="cursor:pointer;" onclick="snet();"/> &nbsp;
<%-- 				<c:if test="${verifyYN.sec_yn != 'S' && (PkgModel.status == '26' || (PkgModel.status > 2 && PkgModel.status < 10))}">
					<c:forEach var="result" items="${SysModel.sel_sysUserSecId}" varStatus="loop">
						<c:if test="${(SysModel.sel_sysUserSecId[loop.count-1]) eq PkgModel.session_user_id}">
							<img src="/images/btn_save.gif" alt="저장" style="cursor:pointer;" onclick="javascript:fn_create_result('${registerFlag}','32','${pkg_verify_gubun}','${QuestFlag}')" />
							<img src="/images/btn_verify_complete.gif" alt="검증완료" style="cursor:pointer;" onclick="javascript:fn_create_result('complete','32','${pkg_verify_gubun}','${QuestFlag}')"/>
						</c:if>
					</c:forEach>
				</c:if> --%>
				</sec:authorize>
		...
		<div id="flow_tb_1" style="display:block; width:98.5%; height:125px; margin-top:50px; overflow:auto; overflow-x:hidden;" align="center">
			<table width="98.5%" class="tbl_type_ly" style="margin-left: 5px;">
				<colgroup>
					<col width="30%">
					<col width="70%">
				</colgroup>
			...
			<caption>일정 등록</caption>
				...
				<tr>
					<td scope="col">보안검증 일정 등록 <span class='necessariness'>*</span></td>
					<td scope="col" style="text-align:left; padding-left:10px;">
						...
						<input type="text" id="col43" name="col43" value="${pkgStatusModel.col43}" style="width:90%;" class="inp" maxlength="100" onkeydown="fn_detail_variable_textarea_check(this);"/>
							<select id="selID43" onchange="fnSelBox43();">
								<c:forEach var="selectItem" begin="1" end="4" step="1">
									<c:choose>
										<c:when test="${PkgModel.file78 == null }">
											<option><c:out value="${selectItem}" /></option>
										</c:when>
										<c:otherwise>
											<c:choose>
												<c:when test="${PkgModel.file79 == null }">
													<option><c:out value="${selectItem}" /></option>
												</c:when>
												<c:otherwise>
													<c:choose>
														<c:when test="${PkgModel.file80 == null }">
														...
														<c:otherwise>
															<c:choose>
																<c:when test="${PkgModel.file81 == null }">
						...
						<td scope="col" align="left">
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<div id="display_43_1"><ui:file attachFileModel="${PkgModel.file78}" name="file78" size="50" mode="update" /></div>
								</c:when>
								<c:otherwise>
									<div id="display_43_1"><ui:file attachFileModel="${PkgModel.file78}" name="file78" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file79 == null }">
											<div id="display_43_2" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file79}" name="file79" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_2"><br/><ui:file attachFileModel="${PkgModel.file79}" name="file79" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_2" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file79}" name="file79" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file80 == null }">
											<div id="display_43_3" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file80}" name="file80" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_3"><br/><ui:file attachFileModel="${PkgModel.file80}" name="file80" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_3" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file80}" name="file80" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${registerFlag == '수정'}">
									<c:choose>
										<c:when test="${PkgModel.file81 == null }">
											<div id="display_43_4" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file81}" name="file81" size="50" mode="update" /></div>
										</c:when>
										<c:otherwise>
											<div id="display_43_4"><br/><ui:file attachFileModel="${PkgModel.file81}" name="file81" size="50" mode="update" /></div>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
									<div id="display_43_4" style="display:none;"><br/><ui:file attachFileModel="${PkgModel.file81}" name="file81" size="50" mode="create" /></div>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
	...
	<div class="edge_cen" style="top:170px;"></div>
