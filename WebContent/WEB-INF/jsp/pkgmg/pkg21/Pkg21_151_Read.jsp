...
//파일 유효성 체크 end
    ...
    data.append("pkgTitle", pkgTitle);
   	...
   	if( paidcall != "lastVolResultAttach"){
...
if(type == "listfile"){
	...
	for(var i=0; i < list.length; i++){
		if(list[i].attach_file_id  != null && "lastVolResultAttach" == list[i].attach_file_id.substring(0,19)){//				ttm 첨부
				++count;
...
$.ajax({
   ...
   data:{"master_file_id" : $("#master_file_id").val(), "type" : "용량검증","use_yn" : "Y"},
   success:function(data){
        var html='';	
        var html2='';
        var flag = true;
// 	        	console.log(data);
        for(var i=0; i < data.length; i++){
			if(data[i].pId != 1000 && data[i].attach_file_id  == null){
				...
				if(data[i].attach_file_id  != null){
					if("lastVolResultAttach" == data[i].attach_file_id.substring(0,19) && flag ){
						var htmlttm =	attachfiles(data[i].id,data,"lastVolResultAttach"); //	ttm 첨부
						html2 +=	htmlttm[0] ;
						html2 += '<input type=\"hidden\" id=\"lastVolResultAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
						flag = false;
					}
				}
			}
		...
		$('#lastVolResultdrop').empty();
		$('#lastVolResultdrop').append(html);
	...
	<div class="sub_contents02"  id="con_list" style="display: block;">
		...
		<div class="table_style01">
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>용량 검증 일자 <span class="txt_red">*</span></th>
						<td>
							<form:input path="col1" class="new_inp inp_w20 fl_left" readOnly="true" />
							<span class="fl_left mg05"> ~ </span>
							<form:input path="col2" class="new_inp inp_w20 fl_left" readOnly="true" />
						</td>
					</tr>
				</tbody>
			</table>		
...
<%-- 								<input type="checkbox" name="chk_seqs" value="${result.chk_seq}" checked/> --%>
							...
							<td class="td_center">
								<c:forEach var="result2" items="${Pkg21Model.chk_result_list}" varStatus="status">
									<input type="radio" id="${result.chk_seq}" name="chk_results${status1.index}" value ="<c:out value='${result2.code}' />" <c:if test="${result2.code == result.chk_result}">checked="checked"</c:if>  onclick="javascript:chk_value();" >
					...
					<!-- <tr>
						...
						<td>
							...
							<span class="mg03 ml05 mr15 fl_left">OK</span>
							<input type="radio" class="fl_left">
							<span class="mg03 ml05 mr15 fl_left">NOK</span>
							<input type="radio" class="fl_left">
							<span class="mg03 ml05 fl_left">PASS</span>
						</td>
					</tr> -->
				</tbody>
			</table>
		</div>
		<div id="lastVolResultdrop" class="table_style03 mt10 line_dotte">
			<p class="mg10 fl_wrap"><span class="mt05 fl_left">검증결과  파일 업로드는 드래그앤드랍으로도 가능합니다. 파일 업로드는 최대 5개입니다.</span> <span class="fl_right btn_line_blue2"><a href="#" class="btn btn_red">파일찾기</a></span><p>
			<table class="con_width100">
				<colgroup>
					<col width="70%">
					<col width="10%">
		...
		<div class="table_style01 mt10">
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>용량검증<br>Comment</th>
						<td>
							<form:textarea path="col3" class="new_inp textarea_w95" maxlength="550" />
						</td>
					</tr>
					<tr>
						<th>검증 담당자</th>
		...
		<div class="table_style03 mt30">
			...
			<div class="mt10 mb10 txt_888">
				<p>* 시스템에 등록된 용량검증 담당자 정보를 현행화하세요.</p>
			</div>
		</div>
		...
		<c:if test="${Pkg21Model.col1 == ''}">
			<c:if test="${not empty Pkg21Model.systemUserModelList}">
				<div class="bt_btn2">
					<span class="btn_org2">
						<a href="javascript:fn_save_151();">용량검증 승인요청</a>
					</span>
				</div>
			</c:if>
		</c:if>
