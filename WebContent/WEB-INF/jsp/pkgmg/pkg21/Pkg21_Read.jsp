...
//파일 유효성 체크 end
        ...
        //20181206 eryoon 신규 등록시 패키지 이름 생성 추가
        var sysUserNm = "${Pkg21Model.system_user_name}";
        var pkgTitle = "${Pkg21Model.title}";
        var ver_gubun = $('[name=ver_gubun]:checked').val();
        var ver_txt = "FULL";
        var ver = $('#ver').val();
        var pkgFileFlag = false;
        ...
        if (pkgTitle == "") {
            ...
            switch (ver_gubun) {
	    		case "F": ver_txt = "FULL"; 	break;
	    		case "P": ver_txt = "Partial"; 	break;
	    		case "C": ver_txt = "Patch"; 	break;
	    	}
        	
	        if (system_name == "") {
	        	alert("시스템을 먼저 선택하십시요!");
	        	pkgFileFlag = false;
	        	$('#system_name').focus();
	        	return;
	        }else{
	        	pkgFileFlag = true;
	        	system_name = system_name.substring(system_name.lastIndexOf('>') + 1)
	        }
	        if (ver == "") {
	        	alert("PKG버전을 입력 하십시요!");
	        	pkgFileFlag = false;
	        	$('#ver').focus();
	        	return;
	        }else{
	        ...
	        if (pkgFileFlag) {
	        	pkgTitle = system_name + "_" + ver + "_" + ver_txt;
	        }
        }else{
        	pkgTitle = "${Pkg21Model.title}";
        }
        ...
        if( paidcall != "ttmattach"){
 		...
 		$.ajax({
 			...
 			data: data,
 			type:'POST',
			/* data: data, *///20181119 eryoon  dup 선언으로 삭제
			async:false,
			cache:false,
			contentType:false,
			processData:false,
...
function attachList(){ //첨부 리스트
	$.ajax({
	   ...
	   data:{"master_file_id" : $("#master_file_id").val(), "type" : "SVT계획수립 첨부","use_yn" : "Y"},
       success:function(data){
	        var html='';	
	        var html2='';
	        var flag = true;
...
var fnLoadList = function () {
	$("#Pkg21Model").attr("action","/pkgmg/pkg21/Pkg21_ReadList.do"); 
	$("#Pkg21Model").submit();
}
...
<div class="new_con_Div32">
	...
	<div class="sub_contents01 fl_wrap">
		<p class="flow_comment"><span class="txt_blue2">●- 파란색</span> 은 <strong>완료단계</strong> <span class="txt_yellow ml05">●- 노란색</span> 은  <strong>현재 진행중</strong>인 단계입니다.</p>
		<p class="pkg_del"><!-- .new_btn_sear a -->
			<a href="javascript:fn_pkg21_delete();"><!-- <span class="ic_del fl_left mr05">x</span> --> PKG 삭제</a>
		</p>
				...
				<li class="sub_flow">
					<c:choose>
						...
						<c:when test="${Pkg21Model.status > 131}">
							<p class="ing" style="cursor: pointer;" onclick="javascript:fn_pkg21_status_read('131');">
								<img src="/images/ic_flow_blue.png">
				...
				<li class="sub_flow">
					<c:choose>
						...
						<c:when test="${Pkg21Model.status > 141}">
							<p class="ing" style="cursor: pointer;" onclick="javascript:fn_pkg21_status_read('141');">
								<img src="/images/ic_flow_blue.png">
					...
					<tbody>
						<tr>
							...
							<td colspan="3">
								...
								<span class="fl_left">
									<form:input path="system_name" maxlength="50" class="new_inp" style="width:500px" readonly="true" />
								</span>
								...
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											...
											<c:when test="${Pkg21Model.dev_yn == 'N'}">
												<span class="mg03 ml05 mr25">CVT 검증요청(DVT 검증생략)</span>
											</c:when>
											<c:otherwise>
						...
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${registerFlag == '등록'}">
										시스템_버전_버전구분 (SVT 계획수립 저장 시 생성됩니다.)
									</c:when>
									<c:otherwise>
...
<%-- 										<form:hidden path="ver_gubun" /> --%>
										<c:choose>
											<c:when test="${Pkg21Model.patch_yn == 'Y'}">
												<span>
													<input type="radio" name="ver_gubun" class="fl_left" id="ver_gubun1" disabled="disabled" value="F"/> 
													<label for="ver_gubun1">Full</label>
												</span>
												<span>	
													<input type="radio" name="ver_gubun" class="fl_left" id="ver_gubun2" disabled="disabled"  value="P"/> 
											...
											<c:otherwise>
												<c:choose>
													<c:when test="${Pkg21Model.ver_gubun == 'F'}">
														...
														<span>
															<input type="radio" name="ver_gubun" class="fl_left" id="ver_gubun3" disabled="disabled" value="C"/> 
															<label for="ver_gubun3">Patch</label>
														</span>
													</c:when>
													<c:otherwise>
...
<%-- 								<span class="mg03 ml05">과금검증&nbsp;<input type="checkbox" class="fl_left" name="vol_yn" onclick="return false;" value="${Pkg21Model.vol_yn}"/></span> --%>
						...
						<tr>
							<th>운용투자</th>
							<td colspan="3">
								<form:textarea path="content_invest" rows="5" class="textarea_w95" maxlength="550" />
							</td>
						</tr>
						<tr>
							<th>PN</th>
...
<!-- 					페이지 로딩시에 스크립트에서 다시 내려받는다. -->
		...
		<div class="bt_btn_wrap fl_wrap">
			...
			<a href="javascript:fn_save_101();" class="bt_btn1 bt_btn_orang3">저장</a>
		</div>
<!-- 		<div class="con_top_btn fl_wrap" style="margin: 20px 0 0 0;">
			<span class="btn_blue2">
				<a href="javascript:fnLoadList();">목록</a>
			</span>
			<span class="btn_org2" onclick="javascript:fn_save_101();">
				<a href="javascript:fn_save_101();">저 장</a>
			</span>
		</div> -->
	</div>
</div>
