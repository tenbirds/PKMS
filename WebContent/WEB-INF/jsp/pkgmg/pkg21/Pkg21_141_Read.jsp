...
<div class="new_con_Div32">
	...
	<!---- /확대적용 계획수립 ---->
	<c:if test="${Pkg21Model.status == 134}">
		<div class="bt_btn">
			<span class="btn_org2">
				<a href="javascript:fn_save_140();">일정 저장</a>
			</span>
		</div>
	</c:if>
	<c:choose>
		<c:when test="${Pkg21Model.status > 141}">
			<div class="bt_btn">
				<span class="btn_org2">
					<a href="javascript:fn_after_eq_e();">일정 저장</a>
				</span>
			</div>
		</c:when>
		<c:when test="${Pkg21Model.status > 134}">
			<div class="bt_btn">
				<span class="btn_org2">
					<a href="javascript:fn_save_eq_e();">일정 저장</a>
				</span>
			</div>
		</c:when>
		<c:otherwise>
	...
	<div class="sub_contents02"  id="con_list1" style="display: block">
			...
			<table class="con_width100">
				...
				<tbody>
					<c:choose>
						...
						<c:otherwise>
							<c:if test="${Pkg21Model.status == 141 && Pkg21Model.update_gubun == 'Y'}">
								<c:forEach var="result" items="${Pkg21Model.systemUserModelList}" varStatus="status">
									<tr>
										<td class="td_center">
											<c:choose>
							...
							<c:if test="${Pkg21Model.status > 134 && Pkg21Model.update_gubun == 'N'}">
								<c:forEach var="result" items="${Pkg21Model.pkgUserModelList}" varStatus="status">
									<tr>
										<td class="td_center">
											<c:if test="${result.status == 'R'}">
		...
		<c:if test="${Pkg21Model.status == 141 && Pkg21Model.update_gubun == 'Y'}">
			<div class="bt_btn">
				<span class="btn_org2">
					<a href="javascript:fn_save_141();">확대승인요청</a>
				</span>
			</div>
		</c:if>
		<c:if test="${Pkg21Model.status > 141 && empty Pkg21Model.pkgUserModelList}">
			<div class="bt_btn">
				<span class="btn_org2">
					<a href="javascript:fn_after_141();">확대승인요청</a>
				</span>
			</div>
		</c:if>
	</div>
