...
function F_FileMultiUpload(files, obj) {
	...
	if(obj.attr("name") == "3SEquipmentAttach"){
	...
	if(confirm(fileCount + "개의 파일을 업로드 하시겠습니까?") ) {
       ...
       if( paidcall != "3SEquipmentAttach" && paidcall != "3hoTestResultAttach" && paidcall != "3systemLevelAttach"){
...
function attachfiles(menuid,list,type){ // 파일 내리기
	...
	if(type == "listfile"){
		...
		for(var i=0; i < list.length; i++){
			if(list[i].attach_file_id  != null && "3SRquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "3SRquipment"){
				++count;
				eq_count++;
			...
			if(list[i].attach_file_id  != null && "3SEquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "3SEquipment"){//				ttm 첨부
					++count;
					html2 +='<div id=\"'+list[i].attach_file_id+'\">';
					html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
					html2 +='</div>';
			...
			if(list[i].attach_file_id  != null && "3hoTestResultAttach" == list[i].attach_file_id.substring(0,19) && type == "3hoTestResult" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
			...
			if(list[i].attach_file_id  != null && "3systemLevelAttach" == list[i].attach_file_id.substring(0,18) && type == "3systemLevel" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
...
function attachList(){ //첨부 리스트
	$.ajax({
	   ...
	   data:{"master_file_id" : $("#master_file_id").val(), "type" : "Access 3차 초도적용","use_yn" : "Y"},
       success:function(data){
	        var html='';	
	        var html2='';
	        var html4='';
	        ...
	        for(var i=0; i < data.length; i++){
				...
				if(data[i].pId != 1000 && data[i].attach_file_id  == null){
					...
					if(data[i].attach_file_id  != null){
						if("3SRquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag4 ){
							var htmlttm =	attachfiles(data[i].id,data,"3SRquipment");
							html4 += ''+htmlttm[0] ;
							
							flag4 = false;
						}
						
						if("3SEquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag1 ){
							var htmlttm =	attachfiles(data[i].id,data,"3SEquipment"); //	cvtcomment
							
							html2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								html2 += '<input name=\"3SEquipmentAttach\" id=\"dropzone0\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" >';
								html2 += '<input type=\"hidden\" id=\"3SEquipmentAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
								html2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'3SEquipmentAttach\' );\">';
							}
							flag1 = false;
							del_count++;
						}
						...
						if("3hoTestResultAttach" == data[i].attach_file_id.substring(0,19) && flag2 ){
							var htmlttm =	attachfiles(data[i].id,data,"3hoTestResult"); //	cvtcomment
							
							htmlcom1 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom1 += '<input name=\"3hoTestResultAttach\" id=\"dropzone1\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom1 += '<input type=\"hidden\" id=\"3hoTestResultAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
								htmlcom1 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'3hoTestResultAttach\' );\">';
							}
							flag2 = false;
						}						
						
						
						if("3systemLevelAttach" == data[i].attach_file_id.substring(0,18) && flag3 ){
							var htmlttm =	attachfiles(data[i].id,data,"3systemLevel"); //	cvtcomment
							
							htmlcom2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom2 += '<input name=\"3systemLevelAttach\" id=\"dropzone2\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom2 += '<input type=\"hidden\" id=\"3systemLevelAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
								htmlcom2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'3systemLevelAttach\' );\">';
							}
							flag3 = false;
						}
					}
			...
			if(htmlcom2.length == 0){		
	 		...
	 		$('#3SEquipmentdrop').empty();
	 		$('#3SEquipmentdrop').append(html2);

	 		$('#3hoTestResultdrop').empty();
	 		$('#3hoTestResultdrop').append(htmlcom1);
	 		
	 		$('#3systemLeveldrop').empty();
	 		$('#3systemLeveldrop').append(htmlcom2);
	 		
	 		$('#3SRquipmentdrop').empty();
	 		$('#3SRquipmentdrop').append(html4);
...
<form:form commandName="Pkg21Model" method="post" onsubmit="return false;">
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">3차 초도적용 계획수립</h2>
	...
	<div class="sub_contents02"  id="con_list" style="display: block">
			...
			<table class="con_width100">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<div id="3SEquipmentdrop" class="mt10">
		...
		<div class="write_info2 fl_wrap">
			등록
			<span class="name2">
				<c:if test="${Pkg21Model.status > 230}">
				${Pkg21Model.reg_plan_user} (${Pkg21Model.reg_date_plan})
				</c:if>
			</span>
		</div>
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.status == 222}">
				<a href="javascript:fn_save_231();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
			<c:if test="${Pkg21Model.status == 231}">
				<a href="javascript:fn_after_231();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
		</div>
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">3차 초도적용 결과</h2>
		<div class="sub_con_close2">
	...
	<div class="sub_contents02"  id="con_list1" style="display: block">
		...
		<div class="table_style03 mt10">
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>대상장비</th>
						<td>
							<div id="3SRquipmentdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
			...
			<table id="radioBoxcount" class="con_width100">
				...
				<tbody>
					<c:choose>
						<c:when test="${Pkg21Model.status > 230}">
			...
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>호시험결과</th>
						<td>
							<div id="3hoTestResultdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
					...
					<tr>
						<th>시스템품질</th>
						<td>
							<div id="3systemLeveldrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
		...
		<div class="table_style03 mt20">
								...
								<td class="td_center">
									<c:if test="${result.status == 'R'}">
										<c:if test="${result.ord == Pkg21Model.user_active_status and result.user_id == Pkg21Model.session_user_id}">
											<span class="btn_wrap">
												<a href="javascript:fn_save_233('${result.ord}');" class="btn btn_red">승인</a>
												<a href="javascript:fn_reject_233();" class="btn btn_blue">반려</a>
											</span>
										</c:if>
									</c:if>
								</td>
			...
			<div class="mt10 mb10 txt_888">
				<p>* 시스템에 등록된 CVT승인 담당자 정보를 현행화하세요.</p>
			</div>
		</div>
		...
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.status == 231}">
				<a href="javascript:fn_save_232();" class="bt_btn1 bt_btn_orang2">승인요청</a>
			</c:if>
		</div>
	</div>
</form:form>
