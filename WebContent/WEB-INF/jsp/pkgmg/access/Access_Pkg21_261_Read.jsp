...
function F_FileMultiUpload(files, obj) {
	...
	if(obj.attr("name") == "2CEquipmentAttach"){
	...
	if(confirm(fileCount + "개의 파일을 업로드 하시겠습니까?") ) {
       ...
       if( paidcall != "2CEquipmentAttach" && paidcall != "2CoTestResultAttach" && paidcall != "2CystemLevelAttach"){
...
function attachfiles(menuid,list,type){ // 파일 내리기
	...
	if(type == "listfile"){
		...
		for(var i=0; i < list.length; i++){
			if(list[i].attach_file_id  != null && "2CRquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "2CRquipment"){
				++count;
				eq_count++;
			...
			if(list[i].attach_file_id  != null && "2CEquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "2CEquipment"){//				ttm 첨부
					++count;
					html2 +='<div id=\"'+list[i].attach_file_id+'\">';
					html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
// 					html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
			...
			if(list[i].attach_file_id  != null && "2CoTestResultAttach" == list[i].attach_file_id.substring(0,19) && type == "2CoTestResult" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
			...
			if(list[i].attach_file_id  != null && "2CystemLevelAttach" == list[i].attach_file_id.substring(0,18) && type == "2CystemLevel" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
...
function attachList(){ //첨부 리스트
	$.ajax({
	   ...
	   data:{"master_file_id" : $("#master_file_id").val(), "type" : "AccessCVT2","use_yn" : "Y"},
       success:function(data){
	        var html='';	
	        var html2='';
	        var html4='';
...
// 						html += '</span>';
					...
					if(data[i].attach_file_id  != null){
						if("2CRquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag4 ){
							var htmlttm =	attachfiles(data[i].id,data,"2CRquipment");
							html4 += ''+htmlttm[0] ;
							
							flag4 = false;
						}
						
						if("2CEquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag1 ){
							var htmlttm =	attachfiles(data[i].id,data,"2CEquipment"); //	cvtcomment
							
							html2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								html2 += '<input name=\"2CEquipmentAttach\" id=\"dropzone0\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" >';
								html2 += '<input type=\"hidden\" id=\"2CEquipmentAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
// 								html2 += '<span class=\"btn_line_blue2 ml10 mt02\">';
								html2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'2CEquipmentAttach\' );\">';
// 								html2 += '</span>';
							}
							flag1 = false;
							del_count++;
						...
						if("2CoTestResultAttach" == data[i].attach_file_id.substring(0,19) && flag2 ){
							var htmlttm =	attachfiles(data[i].id,data,"2CoTestResult"); //	cvtcomment
							
							htmlcom1 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom1 += '<input name=\"2CoTestResultAttach\" id=\"dropzone1\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom1 += '<input type=\"hidden\" id=\"2CoTestResultAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
// 								htmlcom1 += '<span class=\"btn_line_blue2 ml10 mt02\">';
								htmlcom1 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'2CoTestResultAttach\' );\">';
// 								htmlcom1 += '</span>';
							}
							flag2 = false;
						}						
						...
						if("2CystemLevelAttach" == data[i].attach_file_id.substring(0,18) && flag3 ){
							var htmlttm =	attachfiles(data[i].id,data,"2CystemLevel"); //	cvtcomment
							
							htmlcom2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom2 += '<input name=\"2CystemLevelAttach\" id=\"dropzone2\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom2 += '<input type=\"hidden\" id=\"2CystemLevelAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
								htmlcom2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'2CystemLevelAttach\' );\">';
							}
							flag3 = false;
						}
					}
			...
			if(htmlcom2.length == 0){		
	 		...
	 		$('#2CEquipmentdrop').empty();
	 		$('#2CEquipmentdrop').append(html2);

	 		$('#2CoTestResultdrop').empty();
	 		$('#2CoTestResultdrop').append(htmlcom1);
	 		
	 		$('#2CystemLeveldrop').empty();
	 		$('#2CystemLeveldrop').append(htmlcom2);
	 		
	 		$('#2CRquipmentdrop').empty();
	 		$('#2CRquipmentdrop').append(html4);
...
<form:form commandName="Pkg21Model" method="post" onsubmit="return false;">
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">2차 상용적용 계획수립</h2>
	...
	<div class="sub_contents02"  id="con_list" style="display: block">
			...
			<table class="con_width100">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<div id="2CEquipmentdrop" class="mt10">
		...
		<div class="write_info2 fl_wrap">
			등록
			<span class="name2">
				<c:if test="${Pkg21Model.status > 260}">
		...
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.col43 == 'Y'}">
				<a href="javascript:fn_save_261();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
			<c:if test="${Pkg21Model.status == 261}">
				<a href="javascript:fn_after_261();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
		</div>
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">2차 상용적용 결과</h2>
		<div class="sub_con_close2">
	...
	<div class="sub_contents02"  id="con_list1" style="display: block">
		...
		<div class="table_style03">
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>대상장비</th>
						<td>
							<div id="2CRquipmentdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
			...
			<table id="radioBoxcount" class="con_width100">
				...
				<tbody>
					<c:choose>
						<c:when test="${Pkg21Model.status > 260}">
			...
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>호시험결과</th>
						<td>
							<div id="2CoTestResultdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
					...
					<tr>
						<th>시스템품질</th>
						<td>
							<div id="2CystemLeveldrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
					...
					<tr>
						<th>comment</th>
						<td>
							<form:textarea path="col1" rows="5" class="textarea_w95" maxlength="550" />
						</td>
					</tr>
				</tbody>
			</table>
								...
								<td class="td_center">
									<c:if test="${result.status == 'R'}">
										<c:if test="${result.ord == Pkg21Model.user_active_status and result.user_id == Pkg21Model.session_user_id}">
											<span class="btn_wrap">
												<a href="javascript:fn_save_263('${result.ord}');" class="btn btn_red">승인</a>
												<a href="javascript:fn_reject_263();" class="btn btn_blue">반려</a>
											</span>
										</c:if>
									</c:if>
								</td>
		...
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.status == 261}">
				<a href="javascript:fn_save_262();" class="bt_btn1 bt_btn_orang2">승인요청</a>
			</c:if>
		</div>
	</div>
</form:form>
