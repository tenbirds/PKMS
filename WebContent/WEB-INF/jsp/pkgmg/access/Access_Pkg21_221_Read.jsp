...
function F_FileMultiUpload(files, obj) {
	...
	if(obj.attr("name") == "2SEquipmentAttach"){
...
//파일 유효성 체크 end
       ...
       if( paidcall != "2SEquipmentAttach" && paidcall != "2hoTestResultAttach" && paidcall != "2systemLevelAttach"){
...
function attachfiles(menuid,list,type){ // 파일 내리기
	...
	if(type == "listfile"){
		...
		for(var i=0; i < list.length; i++){
			if(list[i].attach_file_id  != null && "2SRquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "2SRquipment"){
				++count;
				eq_count++;
			...
			if(list[i].attach_file_id  != null && "2SEquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "2SEquipment"){//				ttm 첨부
					++count;
					html2 +='<div id=\"'+list[i].attach_file_id+'\">';
					html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
// 					html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
			...
			if(list[i].attach_file_id  != null && "2hoTestResultAttach" == list[i].attach_file_id.substring(0,19) && type == "2hoTestResult" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
			...
			if(list[i].attach_file_id  != null && "2systemLevelAttach" == list[i].attach_file_id.substring(0,18) && type == "2systemLevel" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
...
function attachList(){ //첨부 리스트
	$.ajax({
	   ...
	   data:{"master_file_id" : $("#master_file_id").val(), "type" : "Access 2차 초도적용","use_yn" : "Y"},
       success:function(data){
	        var html='';	
	        var html2='';
	        var html4='';
...
// 						html += '</span>';
					...
					if(data[i].attach_file_id  != null){
						if("2SRquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag4 ){
							var htmlttm =	attachfiles(data[i].id,data,"2SRquipment");
							html4 += ''+htmlttm[0] ;
							
							flag4 = false;
						}
						
						if("2SEquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag1 ){
							var htmlttm =	attachfiles(data[i].id,data,"2SEquipment"); //	cvtcomment
							
							html2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								html2 += '<input name=\"2SEquipmentAttach\" id=\"dropzone0\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" >';
								html2 += '<input type=\"hidden\" id=\"2SEquipmentAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
// 								html2 += '<span class=\"btn_line_blue2 ml10 mt02\">';
								html2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'2SEquipmentAttach\' );\">';
// 								html2 += '</span>';
							}
							flag1 = false;
							del_count++;
						...
						if("2hoTestResultAttach" == data[i].attach_file_id.substring(0,19) && flag2 ){
							var htmlttm =	attachfiles(data[i].id,data,"2hoTestResult"); //	cvtcomment
							
							htmlcom1 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom1 += '<input name=\"2hoTestResultAttach\" id=\"dropzone1\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom1 += '<input type=\"hidden\" id=\"2hoTestResultAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
// 								htmlcom1 += '<span class=\"btn_line_blue2 ml10 mt02\">';
								htmlcom1 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'2hoTestResultAttach\' );\">';
// 								htmlcom1 += '</span>';
							}
							flag2 = false;
						}						
						...
						if("2systemLevelAttach" == data[i].attach_file_id.substring(0,18) && flag3 ){
							var htmlttm =	attachfiles(data[i].id,data,"2systemLevel"); //	cvtcomment
							
							htmlcom2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom2 += '<input name=\"2systemLevelAttach\" id=\"dropzone2\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom2 += '<input type=\"hidden\" id=\"2systemLevelAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
								htmlcom2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'2systemLevelAttach\' );\">';
							}
							flag3 = false;
						}
					}
			...
			if(htmlcom2.length == 0){		
	 		...
	 		$('#2SEquipmentdrop').empty();
	 		$('#2SEquipmentdrop').append(html2);

	 		$('#2hoTestResultdrop').empty();
	 		$('#2hoTestResultdrop').append(htmlcom1);
	 		
	 		$('#2systemLeveldrop').empty();
	 		$('#2systemLeveldrop').append(htmlcom2);
	 		
	 		$('#2SRquipmentdrop').empty();
	 		$('#2SRquipmentdrop').append(html4);
...
<form:form commandName="Pkg21Model" method="post" onsubmit="return false;">
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">2차 초도적용 계획수립</h2>
	...
	<div class="sub_contents02"  id="con_list" style="display: block">
			...
			<table class="con_width100">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<div id="2SEquipmentdrop" class="mt10">
		...
		<div class="write_info2 fl_wrap">
			등록
			<span class="name2">
				<c:if test="${Pkg21Model.status > 220}">
				${Pkg21Model.reg_plan_user} (${Pkg21Model.reg_date_plan})
				</c:if>
			</span>
		</div>
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.status == 212}">
				<a href="javascript:fn_save_221();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
			<c:if test="${Pkg21Model.status == 221}">
				<a href="javascript:fn_after_221();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
		</div>
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">2차 초도적용 결과</h2>
		<div class="sub_con_close2">
	...
	<div class="sub_contents02"  id="con_list1" style="display: block">
			...
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>대상장비</th>
						<td>
							<div id="2SRquipmentdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
			...
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>호시험결과</th>
						<td>
							<div id="2hoTestResultdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
					...
					<tr>
						<th>시스템품질</th>
						<td>
							<div id="2systemLeveldrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
		...
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.status == 221}">
				<a href="javascript:fn_save_222();" class="bt_btn1 bt_btn_orang2">
					완료
				</a>
			</c:if>
		</div>
