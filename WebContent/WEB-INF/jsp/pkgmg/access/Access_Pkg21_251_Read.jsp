...
function F_FileMultiUpload(files, obj) {
	...
	if(obj.attr("name") == "1CEquipmentAttach"){
	...
	if(confirm(fileCount + "개의 파일을 업로드 하시겠습니까?") ) {
       ...
       if( paidcall != "1CEquipmentAttach" && paidcall != "1CoTestResultAttach" && paidcall != "1CystemLevelAttach"){
...
function attachfiles(menuid,list,type){ // 파일 내리기
	...
	if(type == "listfile"){
		...
		for(var i=0; i < list.length; i++){
			if(list[i].attach_file_id  != null && "1CRquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "1CRquipment"){
				++count;
				eq_count++;
			...
			if(list[i].attach_file_id  != null && "1CEquipmentAttach" == list[i].attach_file_id.substring(0,17)  && type == "1CEquipment"){//				ttm 첨부
					++count;
					html2 +='<div id=\"'+list[i].attach_file_id+'\">';
					html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
// 					html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
			...
			if(list[i].attach_file_id  != null && "1CoTestResultAttach" == list[i].attach_file_id.substring(0,19) && type == "1CoTestResult" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
			...
			if(list[i].attach_file_id  != null && "1CystemLevelAttach" == list[i].attach_file_id.substring(0,18) && type == "1CystemLevel" ){//				ttm 첨부
				++count;
				html2 +='<div id=\"'+list[i].attach_file_id+'\">';
				html2 +='<a href=\"#\" onclick=\"javascript:downloadFile(\''+list[i].file_name+'\',\''+list[i].name+'\',\''+list[i].file_path+'\'); return false;\">'+list[i].name+'</a>';
				html2 +='<input type=\"button\" onClick=\"javascript:delNewFile(\'' + list[i].attach_file_id +'\',\''+ $("#master_file_id").val() +'\',\''+ list[i].file_path+'\',\''+list[i].file_name+'\');\" class=\"btn_del\" title=\"삭제\">';
...
function attachList(){ //첨부 리스트
	$.ajax({
	   ...
	   data:{"master_file_id" : $("#master_file_id").val(), "type" : "AccessCVT1","use_yn" : "Y"},
       success:function(data){
	        var html='';	
	        var html2='';
	        var html4='';
...
// 						html += '</span>';
					...
					if(data[i].attach_file_id  != null){
						if("1CRquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag4 ){
							var htmlttm =	attachfiles(data[i].id,data,"1CRquipment");
							html4 += ''+htmlttm[0] ;
							
							flag4 = false;
						}
						
						if("1CEquipmentAttach" == data[i].attach_file_id.substring(0,17) &&  flag1 ){
							var htmlttm =	attachfiles(data[i].id,data,"1CEquipment"); //	cvtcomment
							
							html2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								html2 += '<input name=\"1CEquipmentAttach\" id=\"dropzone0\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" >';
								html2 += '<input type=\"hidden\" id=\"1CEquipmentAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
// 								html2 += '<span class=\"btn_line_blue2 ml10 mt02\">';
								html2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'1CEquipmentAttach\' );\">';
// 								html2 += '</span>';
							}
							flag1 = false;
							del_count++;
						...
						if("1CoTestResultAttach" == data[i].attach_file_id.substring(0,19) && flag2 ){
							var htmlttm =	attachfiles(data[i].id,data,"1CoTestResult"); //	cvtcomment
							
							htmlcom1 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom1 += '<input name=\"1CoTestResultAttach\" id=\"dropzone1\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom1 += '<input type=\"hidden\" id=\"1CoTestResultAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
// 								htmlcom1 += '<span class=\"btn_line_blue2 ml10 mt02\">';
								htmlcom1 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'1CoTestResultAttach\' );\">';
// 								htmlcom1 += '</span>';
							}
							flag2 = false;
						}						
						...
						if("1CystemLevelAttach" == data[i].attach_file_id.substring(0,18) && flag3 ){
							var htmlttm =	attachfiles(data[i].id,data,"1CystemLevel"); //	cvtcomment
							
							htmlcom2 +=	''+htmlttm[0] ;
							if(htmlttm[1] < 5 ){
								htmlcom2 += '<input name=\"1CystemLevelAttach\" id=\"dropzone2\" type=\"text\" class=\"new_inp new_inp_w3 fl_left mb05 mr05\" placeholder=\"여기에 파일을 드래그해주세요\" ">';
								htmlcom2 += '<input type=\"hidden\" id=\"1CystemLevelAttach_count\"'+' name="5" value=\"'+htmlttm[1]+'\" >';
								htmlcom2 += '<input type=\"file\"  size=\"50\" onchange=\"javascript:docfile_upload(this , \'1CystemLevelAttach\' );\">';
							}
							flag3 = false;
						}
					}
			...
			if(htmlcom2.length == 0){		
	 		...
	 		$('#1CEquipmentdrop').empty();
	 		$('#1CEquipmentdrop').append(html2);

	 		$('#1CoTestResultdrop').empty();
	 		$('#1CoTestResultdrop').append(htmlcom1);
	 		
	 		$('#1CystemLeveldrop').empty();
	 		$('#1CystemLeveldrop').append(htmlcom2);
	 		
	 		$('#1CRquipmentdrop').empty();
	 		$('#1CRquipmentdrop').append(html4);
...
<form:form commandName="Pkg21Model" method="post" onsubmit="return false;">
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">1차 상용적용 계획</h2>
	...
	<div class="sub_contents02"  id="con_list" style="display: block">
			...
			<table class="con_width100">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<div id="1CEquipmentdrop" class="mt10">
		...
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.col43 == 'Y'}">
				<a href="javascript:fn_save_251();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
			<c:if test="${Pkg21Model.col43 == 'N'}">
				<a href="javascript:fn_after_251();" class="bt_btn1 bt_btn_orang2">
					저장
				</a>
			</c:if>
		</div>
	...
	<div class="sub_title fl_wrap">
		<h2 class="fl_left">1차 상용적용 결과</h2>
		<div class="sub_con_close2">
	...
	<div class="sub_contents02"  id="con_list1" style="display: block">
		...
		<div class="table_style03">
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>대상장비</th>
						<td>
							<div id="1CRquipmentdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
			...
			<table id="radioBoxcount" class="con_width100">
				...
				<tbody>
					<c:choose>
						<c:when test="${Pkg21Model.status > 220}">
			...
			<table class="con_width100">
				...
				<tbody>
					<tr>
						<th>호시험결과</th>
						<td>
							<div id="1CoTestResultdrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
					...
					<tr>
						<th>시스템품질</th>
						<td>
							<div id="1CystemLeveldrop" class="mt10">
								<input type="text" class="new_inp new_inp_w3 fl_left"> 
								<span class="btn_line_blue2 mg02"><a href="#">첨부파일</a></span>
							</div>
						</td>
		...
		<div class="bt_btn_wrap">
			<c:if test="${Pkg21Model.col43 == 'N'}">
				<a href="javascript:fn_save_252();" class="bt_btn1 bt_btn_orang2">
					완료
				</a>
			</c:if>
		</div>
