...
<script type="text/javaScript" defer="defer">
	...
	$(document).ready(function($) {
		...
		doTable("scrollTable", "tbl_type", "1", "0", [ "40", "100", "190" ]);
	});
	...
	function fn_confirm() {
		if (empno != "" && !(typeof empno == "undefined")) {
			...
			"fn_callback_confirm");
		} else if (indept != "" && !(typeof indept == "undefined")){
			parent.fn_sktuser_popup_detail_callback(indept, indept_name);
		}
		fn_popup_close();
	}
	...
	function fn_readList(mode) {
		...
		doSubmit("SktUserModel", "/usermg/user/SktUser_Ajax_ReadList.do",
	...
	function onClickTree(key, title) {
		indept = key;
		indept_name = title;
							...
							<div class="search">
								<table class="sear_table1">
									<tr>
										...
										<td><form:select path="searchCondition" items="${SktUserModel.searchConditionsList}" itemLabel="codeName" itemValue="code" /> <form:input path="searchKeyword" cssClass="txt"
												onkeypress="if(event.keyCode == 13) { fn_readList('search'); return;}" /></td>
									</tr>
								</table>

								<!--조회버튼 -->
								<div class="btn_sear">
									<a href="javascript:fn_readList('search');"></a>
								</div>
							</div>
														
							<!--테이블, 페이지 DIV 시작 -->
							<div class="con_Div2" style="margin-top: 45px; position: absolute;">
								<div class="fakeContainer" style="width: 350px; height: 380px">
